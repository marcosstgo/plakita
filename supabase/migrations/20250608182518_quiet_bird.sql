/*
  # Fix database schema and user synchronization
  ...
  2. Security
    ...
    - Set up user synchronization between auth.users and public.users
...
-- First, handle the public.users table creation and column additions
DO $$
BEGIN
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'pets' AND column_name = 'tag_id' AND is_nullable = 'NO'
  ) THEN
...
DO $$
BEGIN
  -- Only sync if auth.users has data

/*
  # Corrección del esquema de tags y otras tablas críticas

  1. Agregar columnas faltantes
    - `user_id` en tabla `tags` (CRÍTICO)
    - `created_at` en tabla `tags` si no existe
    - Verificar otras columnas necesarias

  2. Crear índices necesarios
    - Índice para `tags.user_id`
    - Índice para `tags.code` (único)
    - Índices para búsquedas frecuentes

  3. Actualizar políticas RLS
    - Políticas para que usuarios puedan gestionar sus tags
    - Políticas para admin

  4. Crear foreign keys
    - Relación entre tags y users
    - Relación entre tags y pets
*/

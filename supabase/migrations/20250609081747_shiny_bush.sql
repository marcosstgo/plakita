/*
  ...
  1. Functions
    ...
    - Create validate_tag_pet_integrity() to detect and fix orphaned tags/pets
    - Create get_integrity_report() to generate reports without changes
    - Create cleanup_test_data() to clean test data
    - Create fix_specific_tags() to fix specific problematic tags

  2. Security
    - All functions require admin privileges using is_admin_user()
    - Functions are SECURITY DEFINER for proper permissions
*/

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS validate_tag_pet_integrity();
DROP FUNCTION IF EXISTS get_integrity_report();
DROP FUNCTION IF EXISTS cleanup_test_data();
DROP FUNCTION IF EXISTS fix_specific_tags(TEXT[]);
...
-- Función auxiliar para obtener estadísticas de integridad
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    (SELECT COUNT(*)::INTEGER FROM tags),
    (SELECT COUNT(*)::INTEGER FROM tags WHERE activated = true),
    (SELECT COUNT(*)::INTEGER FROM tags WHERE activated = true AND (pet_id IS NULL OR NOT EXISTS (SELECT 1 FROM pets WHERE id = tags.pet_id))),
    (SELECT COUNT(*)::INTEGER FROM pets),
    (SELECT COUNT(*)::INTEGER FROM pets WHERE qr_activated = true),
    (SELECT COUNT(*)::INTEGER FROM pets WHERE qr_activated = true AND NOT EXISTS (SELECT 1 FROM tags WHERE pet_id = pets.id AND activated = true));
